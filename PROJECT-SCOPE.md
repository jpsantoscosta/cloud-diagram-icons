...
- [x] 2. Index generator → icons.json for Azure builtin entries — scripts/build-index.ps1, 04 Aug 2026; 766 sidebar registrations collapse to 714 distinct icons
...
- [x] 10. Icon MCP server on top of icons.json — published as cloud-diagram-icons-mcp; 0.1.3 (current `latest`, 05 Aug 2026) carries the style-allowlist validation, the full 846-icon offline snapshot, and the usage-focused package page, all verified against the published tarball. Unit tests and stdio smoke test pass
...
| Duplicate registrations of one icon | draw.io registers some shapes in several palettes with different search keywords; 52 index entries repeated both a name and a style, so "App Service" returned five indistinguishable options and forced the caller to choose arbitrarily. The generator now keeps one entry per name+style and folds the other palettes' keywords and category names into its tags. Found by using the MCP server rather than by inspecting the index |
| Same-name entries are not automatically duplicates | Before merging, all 69 same-filename groups were checked by fetching each file and comparing viewBox and path geometry: 60 are the same artwork, but 9 differ despite identical filenames, including Azure Logic Apps, which has three distinct versions across `integration`, `internet_of_things`, and `iot`. Merging by name or filename alone would have destroyed real variants, so only exact name+style repeats are collapsed |
| Search results deduplicated by style | The MCP server also collapses results that share a style string, and treats an alias matching several entries of one shape as unambiguous rather than asking the caller to choose. This protects consumers regardless of the index they load |
| Tests must run against the current index | The bundled offline snapshot is a build artefact and went stale between builds, so tests silently validated old data. A `pretest` step refreshes it before every run |
...
| Refresh Action must not open no-op PRs | Generators skip rewriting icons.json and the mirror manifest when only timestamps would change, so pull requests open only for real icon changes. The first CI run of the gap pipeline proved the point by opening one with no upstream change at all |
| Cross-platform reproducibility of generated files | Some upstream SVGs ship with CRLF, so a Windows workstation and the Linux runner committed byte-different copies of identical icons. `.gitattributes` normalises line endings for both, without which every monthly refresh would have looked like a full-mirror rewrite |
| Mirror freshness vs. change tracking | The manifest's per-estate `synced` date marks the last time that estate's contents changed, not the last time it was checked; stamping every run would reintroduce monthly no-op pull requests. The workflow run history is the record of checks |
