...
2. Append `verticalLabelPosition=bottom;verticalAlign=top;` to each style string so labels sit below icons.
3. If a service returns no icon, use a plain rounded rectangle with the service name and tell the user which services were unresolved.
4. Layout: main flow left to right; monitoring below; orthogonal edges with short labels; dashed edges for telemetry; generous spacing; never crop, flip, rotate, or recolour icons.
5. Build with the draw.io MCP tools when available, otherwise emit standard mxfile XML (`mxfile > diagram > mxGraphModel > root`, reserved cells `0` and `1` first, stable readable cell ids).
